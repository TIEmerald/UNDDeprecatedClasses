...
#import "UNDImageHelper.h"

@implementation UNDImageHelper

#pragma mark - Generals
+ (UIImage *)getImageFromURLString:(NSString *)urlString{
    NSURL *downloadingURL = [[NSURL alloc]initWithString:urlString];
    NSData *downloadedData = [NSData dataWithContentsOfURL:downloadingURL];
...
+ (UIImage *)getImageByRemovingColor:(uint)color fromImage:(UIImage *)orignalImage{
    return [UNDImageHelper getImageByRemovingColorsWithMinColor:color maxColor:color fromImage:orignalImage];
}

+ (UIImage *)getImageByRemovingColorsWithMinColor:(uint)minColor maxColor:(uint)maxColor fromImage:(UIImage *)orignalImage{
    return [UNDImageHelper getImageByReplacingColorsWithMinColor:minColor maxColor:maxColor withColor:0 andAlpha:0 fromImage:orignalImage];
}

+ (UIImage *)getImageByReplacingColor:(UIColor *)color withColor:(UIColor *)newColor fromImage:(UIImage *)orignalImage{
    return [UNDImageHelper getImageByReplacingColorsWithMinColor:color maxColor:color withColor:newColor fromImage:orignalImage];
}
...
+ (UIImage *)getImageByReplacingColorsWithMinColor:(UIColor *)minColor maxColor:(UIColor *)maxColor withColor:(UIColor *)newColor fromImage:(UIImage *)orignalImage{
    return [UNDImageHelper anotherGetImageByReplacingColorsWithMinColor:[UNDImageHelper getColorCodeFromColor:minColor] maxColor:[UNDImageHelper getColorCodeFromColor:maxColor]  withColor:[UNDImageHelper getColorCodeFromColor:newColor] andAlpha:1.0f fromImage:orignalImage];
}
...
+ (UIImage *)getImageByReplacingColorsWithMinColor:(NSUInteger)minColor maxColor:(NSUInteger)maxColor withColor:(NSUInteger)newColor andAlpha:(float)alpha fromImage:(UIImage *)orignalImage{
    CGImageRef imageRef = [UIImage imageWithData:UIImageJPEGRepresentation(orignalImage, 1.0)].CGImage;
...
// Reference from : http://stackoverflow.com/questions/11914137/uicolor-to-unsigned-integer
+ (NSUInteger)getColorCodeFromColor:(UIColor *)aColor
{
    CGFloat red, green, blue;
    if ([aColor getRed:&red green:&green blue:&blue alpha:NULL])
    {
...
+ (UIImage *)anotherGetImageByReplacingColorsWithMinColor:(NSUInteger)minColor maxColor:(NSUInteger)maxColor withColor:(NSUInteger)newColor andAlpha:(float)alpha fromImage:(UIImage *)orignalImage{
    ...
    uint maxGreen = COLOR_PART_GREEN(maxColor);
    uint maxBlue = COLOR_PART_BLUE(maxColor);
    uint newRed = COLOR_PART_RED(newColor);
    uint newGreen = COLOR_PART_GREEN(newColor);
    uint newBlue = COLOR_PART_BLUE(newColor);
    
    
    CGRect imageRect = CGRectMake(0, 0, orignalImage.size.width * precise, orignalImage.size.height * precise);
